...
export const populateMoonPhaseCalendar = async () => {
	try {
		await db.transaction((txn) => {
			txn.executeSql(
				'CREATE TABLE IF NOT EXISTS moonPhase (id bigint NOT NULL PRIMARY KEY, phase_number int NOT NULL, phase_name character varying NOT NULL, phase_image_url character varying NOT NULL, phase_description character_varying NOT NULL)',
				[],
				(tx, results) => {
					console.log('SUCCESS', results);
				},
		...
		await db.transaction((txn) => {
			txn.executeSql(
				'SELECT CAST(id as TEXT) as id, phase_number, phase_name, phase_image_url, phase_description FROM moonPhase ',
				[],
				(tx, results) => {
					if (results.rows.length > 0) {
						const res = results.rows;
